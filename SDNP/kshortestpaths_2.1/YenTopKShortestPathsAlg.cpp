...
void YenTopKShortestPathsAlg::clear()
{
    ...
    for (auto pair : m_mpDerivationVertexIndex)
	{
		delete pair.first;
	}
...
void YenTopKShortestPathsAlg::_init()
{
	...
	m_sub_path_of_derivation_pt.reserve(m_pGraph->getOriginalVertexNum() / 10);
	m_cur_result_sub_path_of_derivation_pt.reserve(m_pGraph->getOriginalVertexNum() / 10);
}
...
BasePath* YenTopKShortestPathsAlg::next()
{
	...
	sub_path_of_derivation_pt.reserve(m_pGraph->getOriginalVertexNum() / 10);*/
	m_sub_path_of_derivation_pt.clear();
	
	
	cur_path->SubPath(m_sub_path_of_derivation_pt, cur_derivation_pt);
	int sub_path_length = m_sub_path_of_derivation_pt.size();
	...
	for (int i=0; i<count-1; ++i)
	{
		...
		cur_result_sub_path_of_derivation_pt.reserve(m_pGraph->getOriginalVertexNum() / 10);*/
		m_cur_result_sub_path_of_derivation_pt.clear();
		
		if (!cur_result_path->SubPath(m_cur_result_sub_path_of_derivation_pt, cur_derivation_pt)) continue;

		if (sub_path_length != m_cur_result_sub_path_of_derivation_pt.size()) continue;
		
		bool is_equal = true;
		for (int i=0; i<sub_path_length; ++i)
		{
			if (m_sub_path_of_derivation_pt.at(i) != m_cur_result_sub_path_of_derivation_pt.at(i))
			{
				is_equal = false;
				break;
			}
	...
	for(int i=path_length-2; i>=0 && !is_done; --i)
	{
		...
		m_pGraph->recover_removed_vertex(cur_recover_vertex->getID()); //Famini change-new recovery
		//reverse_tree.updateAllPrecedentVerticesAfterRecovery(cur_recover_vertex);
		...
		BasePath* cost_forward_path = reverse_tree.update_cost_forward(cur_recover_vertex);

		//4.4 Get one candidate result if possible
		if (cost_forward_path != NULL)
		{
			++m_nGeneratedPathNum;
			...
			//
			for (int j=0; j<cost_forward_path->length(); ++j)
			{
				pre_path_list.push_back(cost_forward_path->GetVertex(j));
			}

			//4.4.2 Compose a candidate and delete the no more used cost_forward_path
			BasePath* sub_path = new Path(pre_path_list, cost + cost_forward_path->Weight());
			delete cost_forward_path;
