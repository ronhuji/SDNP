...
Graph::Graph( const Graph& graph )
{
	...
	m_stMyRemovedVertexIds.insert(graph.m_stMyRemovedVertexIds.begin(), graph.m_stMyRemovedVertexIds.end());
}
...
double Graph::get_edge_weight( const BaseVertex* source, const BaseVertex* sink )
{
	...
	if (isVertexRemoved(source_id)
		|| isVertexRemoved(sink_id)
		|| isEdgeRemoved(make_pair(source_id, sink_id)))
	{
		return DISCONNECT;
	}else
	{
...
void Graph::get_adjacent_vertices( BaseVertex* vertex, set<BaseVertex*>& vertex_set )
{
	...
	if (!isVertexRemoved(starting_vt_id))
	{
			...
			pos != (*vertex_pt_set).end(); ++pos)
		{
			...
			if (isVertexRemoved(ending_vt_id)
				|| isEdgeRemoved(make_pair(starting_vt_id, ending_vt_id)))
			{
				continue;
			}
			//
...
{
	if (!isVertexRemoved(vertex->getID()))
	{
