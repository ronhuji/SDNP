...
from input_scalability_50_short_switches import *
...
def add_base_dicts_values(original_policies_dict, new_policies_dict, generated_policies_dict):
    for (src, dst) in generated_policies_dict:
        ...
        """
        src_parents = get_parents(src_ip_hierarchy_dict, src)
        dst_parents = get_parents(dst_ip_hierarchy_dict, dst)
        for (base_src, base_dst) in itertools.product(src_parents, dst_parents):
            add_from_base(src, dst, base_src, base_dst, original_policies_dict, generated_policies_dict)
            if new_policies_dict != original_policies_dict:
                add_from_base(src, dst, base_src, base_dst, new_policies_dict, generated_policies_dict)

                
def add_from_base(src, dst, base_src, base_dst, base_policies_dict, generated_policies_dict):
    if (base_src, base_dst) in base_policies_dict:
...
def building_policies_main():
    ...
    #print "after ip_hierarchy_dict: ", datetime.datetime.now().time()
    """print src_ip_hierarchy_dict
    print dst_ip_hierarchy_dict
    return"""
    """
    ...
    print "after dags: ", datetime.datetime.now().time()
    flat_generalized_dict = flatten_dict(generalized_src_dst_policies)
    
    print "before new policies: ", datetime.datetime.now().time()
    ...
    generate_additional_policies2(src_dst_policies, new_src_dst_policies, flat_generalized_dict, new_generated_policies_dict)
