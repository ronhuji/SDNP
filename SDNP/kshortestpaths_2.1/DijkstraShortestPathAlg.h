...
typedef std::map<BaseVertex*, VertexVectorPtr> Dag;
...
private: // members
	...
	DagPtr m_mpPredecessorVertices;
...
public:
	DijkstraShortestPathAlg(Graph* pGraph) : m_pDirectGraph(pGraph), m_mpPredecessorVertices(new Dag())
	{
	...
	DagPtr get_shortest_path_flower(BaseVertex* root)
	{
		determine_shortest_paths(NULL, root, false);
		return m_mpPredecessorVertices;
	}
