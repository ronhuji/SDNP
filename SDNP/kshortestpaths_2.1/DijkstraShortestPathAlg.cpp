...
void DijkstraShortestPathAlg::improve2vertex( BaseVertex* cur_vertex_pt, bool is_source2sink )
{
	...
	if(is_source2sink)
	{
		m_pDirectGraph->get_adjacent_vertices(cur_vertex_pt, neighbor_vertex_list);
	}else
	{
		m_pDirectGraph->get_precedent_vertices(cur_vertex_pt, neighbor_vertex_list);
	}

	// 2. update the distance passing on the current vertex
	for(set<BaseVertex*>::iterator cur_neighbor_pos=neighbor_vertex_list.begin(); 
		cur_neighbor_pos!=neighbor_vertex_list.end(); ++cur_neighbor_pos)
	{
		...
		if (cur_pos == m_mpStartDistanceIndex.end() || cur_pos->second > distance)
		{
			...
			(*m_mpPredecessorVertices)[*cur_neighbor_pos] = make_shared<VertexVector>(1, cur_vertex_pt);
		...
		//famini addition to find multiple paths
		else if (cur_pos->second == distance)
		{
			(*m_mpPredecessorVertices)[*cur_neighbor_pos]->push_back(cur_vertex_pt);
		}
	}
}
...
BasePath* DijkstraShortestPathAlg::update_cost_forward( BaseVertex* vertex )
{
	...
	set<BaseVertex*> adj_vertex_set; //todo memory leak: avoid pointer
	...
	while(!vertex_pt_list.empty())
 	{
		...
		/*updatePrecedentVertices(cur_vertex_pt);
		set<BaseVertex*>& pre_vertex_set = *m_vStPrecedentVertices[cur_vertex_pt->getID()];*/ //todo: delete later if i wont use it
